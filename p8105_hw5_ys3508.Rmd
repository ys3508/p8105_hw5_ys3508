...
t_results_df = 
  expand_grid(
    ture_mu = c(1, 2, 3, 4, 5, 6),
    iteration = 1:5000
  ) %>% 
  mutate(
    estimate_df = map(.x = ture_mu, ~ t_test (mu = .x))
  ) %>% 
  unnest(estimate_df)
...
t_plot1 %>% 
    mutate(text_label = str_c("Reject number: ", reject, "\nProportion of rejection: ", prop_reject_null)) %>%
    plot_ly(
    x = ~ ture_mu, y = ~ prop_reject_null, type = "scatter", mode = "markers", 
    text = ~text_label, alpha = 0.5) %>% 
    layout(title = 'The relashinship between power of the test and ture μ', plot_bgcolor = "#e5ecf6", xaxis = list(title = 'Ture μ value'), 
         yaxis = list(title = 'The power of the test)'))
...
t_plot2 %>% 
    plot_ly(
    x = ~ ture_mu, y = ~ avg_estimate_mu, type = "scatter", mode = "markers", 
    alpha = 0.5) %>% 
    layout(title = 'The relashinship between average estimate of μ_hat and ture μ', plot_bgcolor = "#e5ecf6", xaxis = list(title = 'Ture μ value'), 
